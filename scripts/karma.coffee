...
#   hubot karma <thing> add <number> - add arbitrary number of karma to thing
#   hubot karma <thing> subtract <number> - subtract arbitrary number of karma from thing
...
module.exports = (robot) ->
  ...
  robot.hear /(\S+[^+:\s])\+\+(\s|$)/, (msg) ->
    subject = msg.match[1].toLowerCase().replace /^@/g, ""
  ...
  robot.respond /karma (\S+[^-\s]) add (\d+)$/i, (msg) ->
    subject = msg.match[1].toLowerCase().replace /^@/g, ""
    amount = parseInt msg.match[2]
    if subject != "best" && subject != "worst"
      karma.add subject, amount
      msg.send "#{subject} gained #{amount} karma (Karma: #{karma.get(subject)})"

  robot.respond /karma (\S+[^-\s]) subtract (\d+)$/i, (msg) ->
    subject = msg.match[1].toLowerCase().replace /^@/g, ""
    amount = parseInt msg.match[2]
    if subject != "best" && subject != "worst"
      karma.subtract subject, amount
      msg.send "#{subject} lost #{amount} karma (Karma: #{karma.get(subject)})"
